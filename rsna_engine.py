...
def run_iteration(
    ...
    for j in range(inner_iterations):
        for patient_ind, patient_id in enumerate(batch_patient_ids):
            ...
            patches = np.array(patches[:max_num_patches])
            patch_is = np.array([i for i in range(len(patches))])
            shuffle(patch_is)
            for p in range(0, len(patches), patches_per_in_iter):
                z_matrix = z_matrix.detach()
                b_patches = patches[patch_is[p:p+patches_per_in_iter]]
                torch_image = torch.from_numpy(np.stack(b_patches, axis=0)).float().to(device)
                z = q1_model(torch_image)
                z_matrix[patient_ind, patch_is[p:p+len(b_patches)]] = z
